...
const AdminList: FC = () => {
    ...
    const handleRemoveItem = (e: any) => {
        ...
        const filteredItems = allItems.filter((item) => item.id !== Number(targetId));
    ...
    const renderItemList = (item: ISingleItem) => {
        return (
            <div key={item.id} className='item-card relative'>
                <div className='card-remove' id={String(item.id)} onClick={handleRemoveItem}>
                ...
                <Link to={`/${item.barcode}`} style={{textDecoration: 'none'}}>
                    <p><span>{item.brand}</span> {item.name}</p>
                </Link>
                
                <div className='item-data'>
                    <ul id='description' className='data-list'>
                        <li className='list-unmarked'>Штрихкод: <span>{item.barcode}</span></li>
                        <li className='list-unmarked'>Производитель: <span>{item.manufacturer}</span></li>
                        <li className='list-unmarked'>Бренд: <span>{item.brand}</span></li>
                        <li className='list-unmarked'>Тип ухода: <span>{item.care}</span></li>
                    </ul>
                </div>
