...
const AdminEditCard: FC = () => {
    ...
    const selectedCard: any = matchedCard ? matchedCard : blankCard;

    const handleSaveData = (e: any): void => {
        e.preventDefault();
        const formData =  new FormData(e.target);
        const fields = formData.entries();

        const clone = Object.assign({}, selectedCard);
        ...
        for (const [key, value] of fields) {
            ...
            clone[key] = typeof selectedCard[key] === 'number' ? Number(value) : value;
        }
            ...
            <form id='card-form' action="" name='item-card' onSubmit={handleSaveData}>
                <div className='card-data'>
                    ...
                    <div className='card-form-inputs'>
                        <input type="number" name='id' id='item-id' defaultValue={String(selectedCard.id)}/>
                        <input type="text" name='image' id='item-img' defaultValue={String(selectedCard?.image)}/>
                        <input type="text" name='name' id='item-name' defaultValue={String(selectedCard?.name)}/>
                        <input type="text" name='format' id='item-size-format' defaultValue={String(selectedCard?.format)}/>
                        <input type="number" name='size' id='item-size' defaultValue={String(selectedCard?.size)}/>
                        <input type="number" name='barcode' id='item-barcode' defaultValue={String(selectedCard?.barcode)}/>
                        <input type="text" name='manufacturer' id='item-manufacturer' defaultValue={String(selectedCard?.manufacturer)}/>
                        <input type="text" name='brand' id='item-brand' defaultValue={String(selectedCard?.brand)}/>
                        <input type="text" name='description' id='item-description' defaultValue={String(selectedCard?.description)}/>
                        <input type="number" name='price' id='item-price' defaultValue={String(selectedCard?.price)}/>
