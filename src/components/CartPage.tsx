...
import { itemAmountAdded, itemAmountSubtracted, itemDeletedFromCart, totalSumCounted } from '../store/reducers/cartSlice';
import { ICartItem } from '../models/ICartItem';
import { Link, useLocation } from 'react-router-dom';
import Modal from './ModalOrderProcessed';

interface IModalActiveProp {
...
const CartPage: FC = () => {
    ...
    if (cartItems.length !== 0) localStorage.setItem('cartItem', JSON.stringify(cartItems))
            ...
            <div className='cart-item' key={index}>
                ...
                <div className='item-main'>
                    <div className='item-weight'>
                        <img src={item.format === 'мл' ? bottleIcon : boxIcon} alt="Иконка бутылки или коробки" />
                ...
                <div className='quantity-control'>
                    <button className='btn-subtract-amount' onClick={() => handleSubtractItem(item.id, item.addedAmount)}>-</button>
                    <span>{item.addedAmount}</span>
                    <button className='btn-add-amount' onClick={() => handleAddItem(item.id)}>+</button>
                </div>
                <span className='item-price'>{(item.price * item.addedAmount).toFixed(2)} ₽</span>
                ...
                <div className='cart-content'>
                    <div className='breadcrumbs-nav-wrapper'>
                        ...
                        <ul className='breadcrumbs-nav'>
                            ...
                            <Link to={pathname} className="breadcrumbs-nav-link link-plain active">Корзина</Link>
                        </ul>
                    </div>
